...
% USER CONFIGURATION 
rootFolder  = '/Users/martaguzman/Library/CloudStorage/OneDrive-SharedLibraries-KTH/Seraina Dual - 2026_INT_Marta_Guzman_WearableBiofeedback/Recordings/NewRecordingsOpenEarable';
fsPpg       = 84;              % Sampling frequency (Hz)
fsImu       = 50;              % Accel frequency (Hz)
fsGyro      = 50;              % Gyro frequency (Hz)
ppgfilterRange = [0.5 7.0];
imufilterRange = [0.5 3.5];       % TODO: ver si poner 0.3-5 o 0.5-3.5
...
for i = 1:length(folders)
    ...
    for k = 1:length(accFiles)
        try
            ...
            % 1. Preprocess
            [filtAxes, rawAxes] = preprocess_imu_axes(accTable, fsImu, imufilterRange);
            ...
            hTimeAcc = plot_imu_time_comparison(accTime, rawAxes, filtAxes, baseName, imufilterRange);
            save_plot_as_fig(hTimeAcc, sessionPath, baseName, '_acc_t_3axes_Raw_Filt');
            
            hFreqAcc = plot_imu_fft_comparison(fVecAcc, magRawAcc, magFiltAcc, baseName, imufilterRange);
    ...
    for k = 1:length(gyroFiles)
        try
            ...
            [filtGyro, rawGyro] = preprocess_imu_axes(gyroTable, fsGyro, imufilterRange);
            ...
            hTimeG = plot_gyro_time_comparison(gyroTime, rawGyro, filtGyro, baseName, imufilterRange);
            save_plot_as_fig(hTimeG, sessionPath, baseName, '_gyro_t_3axes_Raw_Filt');
            
            hFreqG = plot_gyro_fft_comparison(fVecG, magRawG, magFiltG, baseName, imufilterRange);
